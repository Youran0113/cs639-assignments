...
def get_args():
    ...
    parser.add_argument("--train", type=str, default="data/cfimdb-train.txt") #data/sst-train.txt data/cfimdb-train.txt
    parser.add_argument("--dev", type=str, default="data/cfimdb-dev.txt") #data/sst-dev.txt data/cfimdb-dev.txt
    parser.add_argument("--test", type=str, default="data/cfimdb-test.txt") #data/sst-test.txt data/cfimdb-test.txt
    parser.add_argument("--emb_file", type=str, default="glove.300d.txt") #glove.6B.50d.txt
...
def evaluate(dataset, model, device, tag_vocab=None, filename=None):
    ...
    overall_acc = acc / len(predicts)
    print(f'  -Accuracy: {overall_acc:.4f} ({acc}/{len(predicts)})')

    lines = []
    lines.append(f"Overall Accuracy: {overall_acc:.4f} ({acc}/{len(predicts)})")
    lines.append("-" * 65)
    lines.append(f"{'Class':<8} {'Acc':<10} {'Correct':<10} {'Gold_Count':<12} {'Pred_Count':<10}")

    all_tags = sorted(list(set(class_gold.keys()) | set(class_pred.keys())))

    for t in all_tags:
        label_name = tag_vocab.id2word[t] if tag_vocab else str(t)
        c = class_correct[t]
        g = class_gold[t]
        p = class_pred[t]
        current_acc = (c / g) if g > 0 else 0.0

        lines.append(f"{label_name:<8} {current_acc:.4f}     {c:<10} {g:<12} {p:<10}")
    ...
    try:
        ...
        if filename:
            print(f"  -Detailed report saved to result.txt")
    except Exception as e:
        print(f"Error writing to result.txt: {e}")

    if filename and tag_vocab:
        with open(filename, 'w') as f:
            for y_pred in predicts:
                tag = tag_vocab.id2word[y_pred]
                f.write(f'{tag}\n')
...
def main():
    ...
    class_counts = np.bincount(train_labels)  # number of samples per class
